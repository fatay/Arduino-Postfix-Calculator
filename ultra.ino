...
/*
String goToResult(String Infix,String Postfix) {
  const int operandCounter = operatorCounter + 1;
  String operandArray[operandCounter];
  ...
  for(int k=0; k<Infix.length(); k++){
    char sc = Infix.charAt(k);  
    if(!is_operator(sc)) {
      temp += sc;
    }else{
      operandArray[plus] = temp;
      temp = "";
      plus++;
    }
  }
  ...
  for(int i=0; i<operandCounter; i++) {
     int index = Postfix.indexOf(operandArray[i]);
     int l = operandArray[i].length();
     if(index==0){
        Postfix = Postfix.substring(0,l)+","+Postfix.substring(l,Postfix.length());
     }else {
        Postfix = Postfix.substring(0,index)+","+Postfix.substring(index,index+l)+","+Postfix.substring(index+l,Postfix.length());
